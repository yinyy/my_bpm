...
namespace BPM.Admin.PublicPlatform
{
    ...
    public class Index : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            {
                writer.WriteLine(string.Format("fasdfasdfasfda"));
            }
            ...
            using (StreamWriter writer = new StreamWriter(context.Server.MapPath("~/App_Data/Params-" + DateTime.Now.Ticks + ".txt")))
            {
                writer.WriteLine(string.Format("signature={0}&timestamp={1}&nonce={2}&echostr={3}", signature, timestamp, nonce, echostr));
            }

            if (!AccessTokenContainer.CheckRegistered(Toolkit.AppId))
            {
                AccessTokenContainer.Register(Toolkit.AppId, Toolkit.Secret);
            }

            if (context.Request.HttpMethod.ToUpper() == "GET")
            {
                if (CheckSignature.Check(signature, timestamp, nonce, Toolkit.Token))
                {
