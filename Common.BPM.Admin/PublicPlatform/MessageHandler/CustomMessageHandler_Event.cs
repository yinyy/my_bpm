...
© 2016 Senparc", codeRecord.Version);
        ...
        {
            var message = ResponseMessageBase.CreateFromRequestMessage<ResponseMessageText>(requestMessage);
            if (!string.IsNullOrEmpty(requestMessage.EventKey))
            {
                string senceId = requestMessage.EventKey;
                int deviceId = Convert.ToInt32(senceId);

                //查找设备信息
                AgricultureDeviceModel device = AgricultureDeviceBll.Instance.Get(deviceId);
                if (device == null)
                {
                    message.Content = "参数错误，没有找到指定的设备。";
                }
                else
                {
                    //查看是否已经关注设备
                    AgricultureSubscriberModel subscriber = AgricultureSubscriberBll.Instance.Get(WeixinOpenId, deviceId);
                    if (subscriber == null)
                    {
                        subscriber = new AgricultureSubscriberModel();
                        subscriber.OpenId = WeixinOpenId;
                        subscriber.DeviceId = deviceId;

                        AgricultureSubscriberBll.Instance.Add(subscriber);
                    }

                    message.Content = "已关注设备。";
                }
            }
            else
            {
